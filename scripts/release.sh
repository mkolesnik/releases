...
function create_pr() {
    ...
    git add "${file}"
    git commit -s -m "${msg}"
    #dryrun git push -f "https://${GITHUB_ACTOR}:${GITHUB_TOKEN}@github.com/${ORG}/${project}.git" "HEAD:${branch}"
    dryrun git push -f "https://${GITHUB_TOKEN}:x-oauth-basic@github.com/${ORG}/${project}.git" "HEAD:${branch}"
    pr_to_review=$(dryrun gh pr create --repo "${repo}" --head "${branch}" --base "${base_branch}" --title "${msg}" --body "${msg}")
    dryrun gh pr merge --auto --repo "${repo}" --squash "${pr_to_review}" \
